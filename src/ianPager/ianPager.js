    ...
    if (btnname == "prev" || btnname == "first") {
        ...
        if (imgButton) {
            if (!disabled) {
                ...
                {
                    html.push(" href=\"javascript:" + this.Format(this.CustomJsPageFunction(),pageIndex) +";\">");
                }
                else
                {
                    html.push(" href=\"" + this.BuildUrlString(pageIndex) + "\">");
                }
                html.push("<img border=\"0\" align=\"" + this.ButtonImageAlign() + "\" src=\"" + this.ImagePath() + btnname + this.ButtonImageNameExtension() + this.ButtonImageExtension() + "\" />");
                html.push("</a>");
            }
            else {
                html.push("<img border=\"0\" align=\"" + this.ButtonImageAlign() + "\" src=\"" + this.ImagePath() + btnname + this.ButtonImageNameExtension() + this.ButtonImageExtension() + "\" />");
...
ianPager.prototype.CreateNumericButton = function(pageIndex)
{
    ...
    if (this.PagingButtonType() == "Image" && this.NumericButtonType() == "Image")
    {
        if (!isCurrent)
        {
            ...
            html.push(">");
            html.push(this.CreateNumericButton(pageIndex));
            html.push("</a>");
        }
        else
        {
...
{
    var html = [];
    html.push("<img border=\"0\" align=\""+ this.ButtonImageAlign() +"\" src=\""+ this.ImagePath() + index + ((isCurrent == true) ? this.CpiButtonImageNameExtension():this.ButtonImageNameExtension()) + this.ButtonImageExtension() +"\" />");
    return html.join('');
}
...
ianPager.prototype.BuildUrlString = function(col) {
    ...
    if (this.urlParams == null) {
        ...
        return this.currentUrl + "?" + tempstr.substr(1) + this.currentHash;
    }
    ...
    return this.currentUrl + "?" + sb.join('').substr(1) + this.currentHash;
}
